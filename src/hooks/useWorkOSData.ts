...
export function useWorkOSData() {
  ...
  // Save to local cache key per user
  ...
  const fetchData = useCallback(async () => {
    ...
    if (!isRealUser) {
      // Load local data or initial seed data
    ...
    // Real Supabase Cloud Data Fetching (Guarded with valid UUID)
    try {
      ...
      let loadedP = (pRes.data as Project[]) || [];
      let loadedT = (tRes.data as Task[]) || [];
      let loadedSt = (stRes.data as Subtask[]) || [];
      let loadedI = (iRes.data as Idea[]) || [];
      let loadedIn = (inRes.data as InboxItem[]) || [];
      let loadedN = (nRes.data as Note[]) || [];
      let loadedA = (aRes.data as ActivityLog[]) || [];

      // If user has 0 data in cloud, initialize with seed data!
      if (loadedP.length === 0 && loadedT.length === 0) {
        const seeded = getInitialSeedData(userId);
        await Promise.all([
        ...
        loadedP = seeded.projects;
        loadedT = seeded.tasks;
        loadedSt = seeded.subtasks;
        loadedI = seeded.ideas;
        loadedIn = seeded.inboxItems;
        loadedN = seeded.notes;
      }
      ...
      saveLocalState(loadedP, loadedT, loadedSt, loadedI, loadedIn, loadedN);
      setSyncStatus('synced');
    } catch (err) {
  ...
  const createProject = async (data: Omit<Project, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'progress'>) => {
    const newProj: Project = {
      ...data,
      id: isRealUser ? undefined : 'proj-' + Date.now(), // Let postgres generate UUID if real
      user_id: userId,
      progress: 0,
      created_at: new Date().toISOString(),
